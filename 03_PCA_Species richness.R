...
SP.richness.df<-data.frame(SegNo=species.distribution.df$SegNo,SP.richness=rowSums(species.distribution.df[,-1]))

# calculate species richness for each stream segment.
...
SP.richness.df$class[SP.richness.df$SP.richness<=2]<-4
SP.richness.df$class[SP.richness.df$SP.richness<=6&SP.richness.df$SP.richness>2]<-3
SP.richness.df$class[SP.richness.df$SP.richness<=10&SP.richness.df$SP.richness>6]<-2
SP.richness.df$class[SP.richness.df$SP.richness>10]<-1
